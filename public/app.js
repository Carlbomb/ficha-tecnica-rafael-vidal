...
function R(){
 ...
 if(T=="insumos"){
  ...
  ${
   ...
   `).join("")
   ||
   '<div class="card muted">Cadastre o primeiro insumo.</div>'
  }
  `
 }
  ...
  ${
   ...
   `).join("")
   ||
   '<div class="card muted">Crie a primeira ficha técnica.</div>'
  }
  `
 }
  ...
  ${
    ...
    <div class="card row mobile">
     ...
     <div>
      ...
      <div class="muted">
       ...
       · Meta 30% ${M(N(f.custo_por_porcao)/.30)}
      </div>
     </div>
   ...
   `).join("")
   ||
   '<div class="card muted">O CMV aparecerá após cadastrar fichas.</div>'
  }
  `
 }
}
...
function FI(i={}){
 ...
 <div class="card form">
  ...
  <div class="muted" id="fcv">
   FC: ${N(i.fc||1).toFixed(3)}
  </div>
  ...
  <div class="muted" id="prv">
   Preço real: ${M(i.preco_real)}
  </div>
  ...
  <button class="primary wide" id="sv">
   Salvar insumo
  </button>
...
function totals(){
 ...
 let total=D.itens.reduce(
  (s,x)=>s+N(x.custo),0
 );

 let por=N(D.porcoes)||1;

 let cp=total/por;

 let pv=N(D.preco_venda);
...
function ED(){
 ...
 <h2>
  ${D.id?"Editar":"Nova"} ficha técnica
 </h2>
 ...
 <div class="card form">
  ...
  <input id="re"
   type="number"
   step=".001"
   placeholder="Rendimento da receita (kg/L)"
  ...
  <div class="muted" id="pp">
   Peso por porção:
   ${N(z.peso).toFixed(3)} kg/L
  </div>
  ...
  ${
   D.itens.map((x,n)=>{
    ...
    <div class="ingredient">
     ...
     <div class="muted">
      ${COD(x.insumo_id)}
      · P. líq. ${N(x.qtd)} ${E(x.unidade)}
      · FC ${N(i.fc||1).toFixed(3)}
      · P. bruto ${pb.toFixed(3)} ${E(x.unidade)}
     </div>

     <div class="muted">
      Compra ${M(i.preco_compra)}/${E(i.unidade||"")}
      · Preço real ${M(i.preco_real)}/${E(i.unidade||"")}
      · <b>Custo ${M(x.custo)}</b>
     </div>

     ${
      x.observacao
      ?`<small>${E(x.observacao)}</small>`
      :""
     }
 ...
 function live(){
  ...
  D.porcoes=$("#po").value;
  D.rendimento_kg=$("#re").value;
  D.preco_venda=$("#pv").value;
