...
document.addEventListener("click",e=>{
 const g=e.target.closest(".nav-group-toggle"); if(g){const atual=g.closest(".nav-group"),abrir=!atual?.classList.contains("open");document.querySelectorAll(".main-nav .nav-group.open").forEach(x=>{if(x!==atual)x.classList.remove("open")});atual?.classList.toggle("open",abrir);return}
 ...
 const t=e.target.closest(".main-nav [data-tab]"); if(t){
   const grupo=t.closest(".nav-group");document.querySelectorAll(".main-nav .nav-group.open").forEach(x=>{if(x!==grupo)x.classList.remove("open")});grupo?.classList.add("open");
