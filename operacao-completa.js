...
export function installOperacaoCompleta(app,pool){
 ...
 app.get("/api/perdas/estoque",ar(async(req,res)=>{const {rows}=await pool.query(`SELECT i.id,i.ingrediente,i.unidade,i.preco_real,COALESCE(SUM(m.quantidade),0)::numeric saldo_atual FROM insumos i LEFT JOIN estoque_movimentacoes m ON m.insumo_id=i.id AND m.empresa_id=$1 AND m.unidade_id=$2 WHERE i.empresa_id=$1 AND i.unidade_id=$2 AND i.ativo=TRUE GROUP BY i.id,i.ingrediente,i.unidade,i.preco_real ORDER BY i.ingrediente`,t(req));res.json(rows)}));
